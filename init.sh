...
su --login ${LUSER} /opt/needlesslaser/bin/run "${GIT_EMAIL}" "${GIT_NAME}" "${GIT_URL}" "${GIT_BRANCH_PARENT}" "${GIT_BRANCH_CHILD}" "${LDAP_USERID}" "${LDAP_PASSWORD}" "${LDAP_EMAIL}" "${GPG_KEY_ID}" &&
true
